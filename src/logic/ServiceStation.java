...
public class ServiceStation {
    ...
    public ServiceStation(int numPumps, int queueSize, SimulationView view, StatisticsPanel stats) {
        this.view = view;
        this.stats = stats;
        ...
        pumps = new Pump[numPumps];
        for (int i = 0; i < numPumps; i++) {
            pumps[i] = new Pump(i + 1, carQueue, mutex, availableAreas, waitingCars, availablePumps, view, this);
        }
    }

    public void startSimulation(int numCars) {
        this.totalCars = numCars;
        this.carsServed = 0;
        this.startTime = System.currentTimeMillis();

        if (stats != null) {
            stats.setTotalCars(numCars);
            stats.logEvent("Simulation started with " + numCars + " cars, " + availablePumps + " pumps.");
        }

        for (Pump pump : pumps) pump.start();

        for (int i = 1; i <= numCars; i++) {
          Car car = new Car(i, carQueue, mutex, availableAreas, waitingCars, view, stats);

            car.start();

            if (stats != null) {
                stats.logEvent(" Car " + i + " arrived.");
                stats.updateWaitingCars(getQueueSize());
            }
        ...
        new Thread(() -> {
            while (carsServed < totalCars) {
                int elapsed = (int) ((System.currentTimeMillis() - startTime) / 1000);
                if (stats != null) stats.updateElapsedTime(elapsed);

                if (stats != null && totalCars > 0) {
                    int percent = (int) ((carsServed * 100.0) / totalCars);
                    stats.updateProgressPercentage(percent);
                }
            ...
            if (stats != null) stats.showSummary(carsServed, totalTime);
        }).start();
    }

    public synchronized void carFinished(int carId, int pumpId) {
        carsServed++;
        if (stats != null) {
            stats.logEvent(" Car " + carId + " finished at Pump " + pumpId);
            stats.updateCarsServed(carsServed);
            stats.updateWaitingCars(Math.max(0, carQueue.size()));
        }
    }
